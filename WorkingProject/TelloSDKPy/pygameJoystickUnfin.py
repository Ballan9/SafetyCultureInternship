...
class FrontEnd(object):
    ...
    BUTTONCONTROLMAP = {PyGameAxis.BACK: XboxControls.BACK,
                        PyGameAxis.START: XboxControls.START,
                        PyGameAxis.XBOX: XboxControls.XBOX}
            ...
            for event in pygame.event.get():
                if event.type == USEREVENT + 1:
                    ...
                    if event.axis in self.AXISCONTROLMAP:
                        if event.axis == self.PyGameAxis.LTHUMBY or event.axis == self.PyGameAxis.RTHUMBY:
                            yAxis = True
                            self.updateControlValue(self.AXISCONTROLMAP[event.axis], self._sortOutAxisValue.value, yAxis)
                        else:

                    elif event.axis == self.PyGameButtons.START
                    else:
                        self.keydown(event.key)
