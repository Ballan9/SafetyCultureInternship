...
xboxControls = [0, 1, 2, 3, 4, 5, 8, 9, 11, 12, 13, 14]
inFlightControls = [0, 1, 2, 3, 11, 12, 13, 14]
joysticks = []
...
class FrontEnd(object):
    ...
    def buttondown(self, button):
        ...
        Arguments:
            key: pygame key
        """
        if int(button) == 11:  # set forward velocity (A Button)
            self.for_back_velocity = S
        elif int(button) == 14:  # set backward velocity
            self.for_back_velocity = -S
        elif int(button) == 2:  # set left velocity
            self.left_right_velocity = -S
        elif int(button) == 3:  # set right velocity
            self.left_right_velocity = S
        elif int(button) == 0:  # set up velocity
            self.up_down_velocity = S
        elif int(button) == 1:  # set down velocity
            self.up_down_velocity = -S
        elif int(button) == 12:  # set yaw clockwise velocity
            self.yaw_velocity = -S
        elif int(button) == 13:  # set yaw counter clockwise velocity
            self.yaw_velocity = S
    ...
    def buttonup(self, button):
        ...
        Arguments:
            key: pygame key
        """
        if int(button) == 11 or int(button) == 14:  # set zero forward/backward velocity
            self.for_back_velocity = 0
        elif int(button) == 2 or int(button) == 3:  # set zero left/right velocity
            self.left_right_velocity = 0
        elif int(button) == 0 or int(button) == 1:  # set zero up/down velocity
            self.up_down_velocity = 0
        elif int(button) == 12 or int(button) == 13:  # set zero yaw velocity
            self.yaw_velocity = 0
        elif button == 5:  # takeoff
            self.tello.takeoff()
