...
flip_front = 0  # Flip front flips forward.
flip_left = 1  # Flip left flips left.
flip_back = 2  # Flip back flips backwards.
flip_right = 3  # Flip right flips to the right.
flip_forward_left = 4  # Flip forward-left flips forwards and to the left.
flip_back_left = 5  # Flip back-left flips backwards and to the left.
flip_back_right = 6  # Flip back-right flips backwards and to the right.
flip_forward_right = 7  # Flip forward-right flips forwards and to the right.
...
class Packet(object):
    ...
    def get_time(self, buf=None):
        ...
        minute = int16(buf[2], buf[3])
        sec = int16(buf[4], buf[5])
        millisec = int16(buf[6], buf[8])
        now = datetime.datetime.now()
        return datetime.datetime(now.year, now.month, now.day, hour, minute, sec, millisec)
...
class FlightData(object):
    ...
    def __str__(self):
        return (
            ...
            (", Fly mode = 0x%02x" % self.fly_mode) +
            (", Battery percentage = %2d%%" % self.battery_percentage) +
            (", Drone battery left = 0x%04x" % self.drone_battery_left) +
            "")
